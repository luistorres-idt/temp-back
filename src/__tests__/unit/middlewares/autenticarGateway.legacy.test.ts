...
import { autenticarGatewayLegacy } from "../../../middlewares/autenticacion/autenticacionGateway.legacy.js";
...
describe("autenticarGatewayLegacy middleware (API Key Legacy)", () => {
    ...
    it("debe retornar 401 si no hay Authorization header", async () => {
        ...
        await autenticarGatewayLegacy(mockReq as AppRequest, mockRes as Response, nextFunction);

        expect(mockRes.status).toHaveBeenCalledWith(401);
        expect(mockRes.json).toHaveBeenCalledWith({
            error: "No autorizado. Credenciales de gateway no proporcionadas o incompletas."
        });
    ...
    it("debe autenticar exitosamente si el token y la MAC coinciden y el gateway está activo", async () => {
        mockReq = {
            header: vi.fn().mockImplementation((name) => {
                if (name === "Authorization") return "Bearer token123";
                return undefined;
